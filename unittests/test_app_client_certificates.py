...
class TestCertificatesAutocomplete:
    async def test_returns_certificate_list(self, mock_aiohttp: aioresponses) -> None:
        ...
        mock_aiohttp.get(
            f"{BASE_URL}/api/Certificates/Autocomplete?count=5&pattern=Scrum", body=json.dumps(payload), status=200
        )
            ...
            "certificateHolders": [
                {
                    ...
                    "expirationYear": None,
                    "avatarImageUrl": "https://example.com/img.jpg",
                }
            ],
            "totalCount": 1,
        }
        mock_aiohttp.get(
            f"{BASE_URL}/api/Certificates/{CERT_ID}/CertificateHolders?pageIndex=0&pageSize=20",
